						...
						foreach ($globos as $globo) {
							...
							if($globo->value==$datoReserva[0]->globo){
								...
								if($tVuelo[0]->tipo_vc==46){
									...
									/*Peso en Libras*/
									$pesoActualLib = $con->consulta("sum(kg_temp) as peso","temp_volar","status=8 and tipopeso_temp=2 and globo_temp = ".$globo->value);
									$pesoActualLib=($pesoActualLib[0]->peso* 0.453592);

									/*Peso en Kg*/
									$pesoActualKg = $con->consulta("sum(kg_temp) as peso","temp_volar","status=8 and tipopeso_temp=1 and globo_temp = ".$globo->value);
...
<script type="text/javascript">
	...
	<?php  if($tVuelo[0]->tipo_vc==46){ ?>
		$("#globo").change(function(){
			...
			var2="volar_usuarios";
				var3="status<>0 and puesto_usu = 4 and id_usu not in(SELECT piloto_temp from temp_volar WHERE hora_temp BETWEEN '"+dehora+"' and '"+ ahora +"' and status=8 and fechavuelo_temp ='"+fecha+"'  and piloto_temp is not null and piloto_temp<>0) ";
	...
	<?php } else{ ?>
		...
		function cargarPilotoActual(globo,dehora,ahora,fecha,piloto){
			...
			var3 += "SELECT piloto_temp from  temp_volar where globo_temp ="+globo+"  and piloto_temp is not null and piloto_temp <> 0)";
			parametros={var1:var1,var2:var2,var3:var3};
	  	$("#piloto").empty().append("<option value=''>Selecciona un Piloto </option>");
			$.ajax({
	      data: parametros,

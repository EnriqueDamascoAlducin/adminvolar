...
//	$datosVuelo = $con->consulta("IFNULL(fechavuelo_temp,' No asignada' as fechavuelo, ")
	...
	$pilotos=$con->consulta("CONCAT(nombre_usu,' ',apellidop_usu,' ',apellidom_usu) as nombre","volar_usuarios vu Inner join
	globosasignados_volar gv on vu.id_usu = gv.piloto_ga","gv.status <> 0 AND gv.reserva_ga =".$reserva);

	$globos=$con->consulta("CONCAT(nombre_globo) as nombre","globos_volar vu Inner join
	globosasignados_volar gv on vu.id_globo = gv.globo_ga","gv.status <> 0 AND gv.reserva_ga =".$reserva); ;

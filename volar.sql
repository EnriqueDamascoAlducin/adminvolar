...
--
-- Servidor: localhost
-- Tiempo de generación: 31-07-2019 a las 19:14:06
...
CREATE  PROCEDURE `registrarPago` (IN `_pago` BIGINT, IN `_reserva` BIGINT, IN `_usuario` INT, IN `_metodo` INT, IN `_banco` INT, IN `_referencia` VARCHAR(200), IN `_cantidad` DOUBLE(10,2), IN `_fechaPago` VARCHAR(30), IN `_usuarioCOncilia` INT, OUT `respuesta` VARCHAR(25))  BEGIN
    ...
    ELSEIF (_usuarioConcilia=0) THEN
        ...
        SET respuesta = 'Registrado|',LAST_INSERT_ID();
    ELSE
...
IF (SELECT (sum(cantidad_bp)+  ) from bitpagos_volar where idres_bp= )>(Select total_temp FROM temp_volar where id_temp  = ) THEN
      SET respuesta = 'ERROR EN PAGO';
    ELSEIF (=0) THEN
      INSERT INTO bitpagos_volar (idres_bp,idreg_bp,metodo_bp,banco_bp,referencia_bp,cantidad_bp,fecha_bp) VALUES (,,,,,,);
        SET respuesta = 'Registrado';
    ELSE
      UPDATE bitpagos_volar SET idconc_bp=, fechaconc_bp= CURRENT_TIMESTAMP WHERE id_bp = ;
        SET respuesta = 'Conciliado';
  END IF;
  IF (SELECT (sum(cantidad_bp)+  ) from bitpagos_volar where idres_bp= )=(Select total_temp FROM temp_volar where id_temp  = ) THEN
    UPDATE temp_volar set status = 7 WHERE id_temp = ;
  ELSE
    UPDATE temp_volar set status = 4 WHERE id_temp = ;
  END IF;
...
(1201, 1, '1200', 'FRANCISCA', 'GUAJARDO', 'alducin.asesori@outlook.com', '5529212556', '015529212556', 30, 1, 1, 38, 1, '2019-07-15', NULL, 5, 16, '2019-09-30', NULL, 'LLEGAN A SITIO', NULL, NULL, NULL, NULL, NULL, '0.00', 7600.00, 0, '0.00', 0, NULL, '2019-07-31 12:07:17', 2);
...
--
ALTER TABLE `temp_volar`
  MODIFY `id_temp` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Llave Primaria', AUTO_INCREMENT=1202;

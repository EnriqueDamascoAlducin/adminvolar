...
-- Tiempo de generación: 26-09-2019 a las 01:18:19
...
-- Procedimientos
--
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `actualizarDireccionesRestaurantes` (IN `_hotel` INT, IN `_nombre` VARCHAR(150), IN `_precion` DECIMAL(10,2), IN `_precioa` DECIMAL(10,2), IN `_restaurant` INT, OUT `respuesta` VARCHAR(15))  BEGIN
	SET respuesta ='';
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `agregarServiciosReservas` (IN `_reserva` BIGINT, IN `_servicio` INT, IN `_tipo` INT, IN `_cantidad` BIGINT, OUT `respuesta` VARCHAR(50))  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `asigarPermisosModulos` (IN `_usuario` INT, IN `_modulo` INT, IN `_status` INT, OUT `_respuesta` VARCHAR(20))  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `eliminarDepartamento` (IN `_depto` INT, OUT `respuesta` VARCHAR(25))  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `eliminarPuesto` (IN `_puesto` INT, OUT `respuesta` VARCHAR(10))  BEGIN 
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `eliminarServicio` (IN `_servicio` INT, OUT `respuesta` VARCHAR(20))  BEGIN
...
IFNULL(fechavuelo_temp,'Fecha No Asignada') as fechavuelo, CONCAT(IFNULL(nombre_temp,''),' ', IFNULL(apellidos_temp,'')) as nombre, IFNULL(mail_temp,'') as correo, CONCAT(IFNULL(telfijo_temp,''),' <br> ', IFNULL(telcelular_temp,'')) as telefonos, nombre_vc as tipoVuelo, precion_vc as precioN, precioa_vc as precioA
FROM temp_volar tv  INNER JOIN vueloscat_volar vcv on vcv.id_vc = tv.tipo_temp
Where id_temp=_reserva;
END$$

CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `getServicioInfo` (IN `_servicio` INT)  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `getServiciosReservas` (IN `_reserva` BIGINT, IN `_tipo` INT, IN `_servicio` INT)  BEGIN
	Select * from servicios_vuelo_temp where idtemp_sv = _reserva  and tipo_sv = _tipo and idservi_sv = _servicio and cantidad_sv>0 ;
END$$

CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `getServiciosXVta` (IN `venta` BIGINT, IN `servicio` INT, OUT `respuesta` INT)  BEGIN 
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `getUsuarioInfo` (IN `_usuario` INT)  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `permisosModulos` (IN `_idusu` INT)  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `permisosSubModulos` (IN `_idusu` INT, IN `_idmodulo` INT)  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `registrarReserva` (IN `_idusu` INT, OUT `lid` BIGINT)  BEGIN
	Insert INTO temp_volar (idusu_temp) VALUES(_idusu);
   	SET lid =  LAST_INSERT_ID();
END$$

CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `registrarRestaurantHotel` (IN `_hotel` INT, IN `_nombre` VARCHAR(250), IN `_precion` DECIMAL(10,2), IN `_precioa` DECIMAL(10,2), OUT `LID` INT)  BEGIN
	INSERT INTO restaurantes_volar (
	nombre_restaurant,
	calle_restaurant,
	noint_restaurant,
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `registrarServicioVta` (IN `servicio` INT, IN `venta` BIGINT, IN `cantidad` INT, OUT `respuesta` VARCHAR(20))  BEGIN 
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `registrarUsuario` (IN `idusu` INT, IN `nombre` VARCHAR(200), IN `apellidop` VARCHAR(200), IN `apellidom` VARCHAR(200), IN `depto` INT, IN `puesto` INT, IN `correo` VARCHAR(250), IN `telefono` VARCHAR(14), IN `contrasena` VARCHAR(200), IN `usuario` VARCHAR(200), OUT `respuesta` VARCHAR(20))  BEGIN
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `remplazarReserva` (IN `_reserva` INT, IN `_idusu` INT, OUT `lid` INT)  BEGIN 
...
CREATE DEFINER=`afraqujt`@`localhost` PROCEDURE `remplazarServiciosReservas` (IN `_reserva` BIGINT, IN `_oldReserva` BIGINT)  BEGIN 
...
(98, 1163, 16, 57, 64, '08590277483032689', 2000.00, '2019-09-25', 17, '2019-09-25 15:33:11', '2019-09-25 15:30:40', 3),
(99, 1161, 11, 89, 64, '778068', 2000.00, '2019-09-25', 17, '2019-09-25 17:14:49', '2019-09-25 17:12:37', 1),
(100, 1166, 16, 57, 64, '0019762006', 2000.00, '2019-09-25', 17, '2019-09-25 17:34:02', '2019-09-25 17:28:42', 3),
(101, 1165, 16, 57, 64, '0054096012', 2000.00, '2019-09-25', 17, '2019-09-25 18:18:32', '2019-09-25 18:15:48', 3),
...
(1150, 9, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0.00, 0.00, 0, 0.00, 1, 0, NULL, '2019-09-24 13:15:05', 0),
(1151, 14, NULL, 'MARU', 'FIGUEROA', 'volarenglobo@yahoo.es', NULL, '5544948848', 11, 2, 2, 36, 4, '2019-10-05', NULL, NULL, NULL, NULL, NULL, 'VUELO PRIVADO PARA 2 ADULTOS 2 MENORES , VUELO SOBRE VALLE DE TEOTIHUACAN DURANTE 45 MINS APROX , COFFE BREAK INCLUIDO (CAFÃ‰,TE,CAFÃ‰,PAN) SEGURO DE VIAJERO,CERTIFICADO PERSONALIZADO,TRANSPORTE DURANTE LA ACTIVIDAD, BRINDIS CON VINO ESPUMOSO, DESAYUNO EN RESTAURANTE GRAN TEOCALLI Y FOTOGRAFIA IMPRESA A ELEGIR.', NULL, NULL, NULL, NULL, 2, 2.00, 10598.00, 0, 0.00, 1, 0, NULL, '2019-09-24 13:21:18', 8),
(1152, 14, NULL, 'MONICA', 'BERMEO', 'volarenglobo@yahoo.es', NULL, '57320987023', 35, 2, 2, 36, 1, '2019-10-12', NULL, NULL, NULL, NULL, NULL, 'VUELO COMPARTIDO PARA 2 ADULTOS 2 MENORES, VUELO SOBRE EL VALLE DE TEOTIHUACAN, DURANTE 45 MINS APROX, INCLUYE COFFE BREAK (CAFÃ‰, TE, PAN GALLETAS) SEGURO DE VIAJERO, CERTIFICADO PERSONALIZADO, TRANSPORTE DURANTE LA ACTIVIDAD, BRINDIS CON VINO ESPUMOSO, DESAYUNO BUFFET ', 'DESAYUNOS', 396.00, NULL, NULL, NULL, 0.00, 8396.00, 0, 0.00, 1, 0, NULL, '2019-09-24 15:35:32', 3),
(1153, 16, NULL, 'DANIELA', 'PEREZ', 'chicdannnn@gmail.com', NULL, '5586764141', NULL, 2, 0, 37, 4, '2019-09-25', NULL, NULL, NULL, NULL, NULL, 'â€¢	Vuelo en globo exclusivo\nâ€¢	Brindis en vuelo con vino espumoso Freixenet\nâ€¢	Certificado de vuelo\nâ€¢	Seguro viajero\nâ€¢	Desayuno tipo Buffet (Restaurante Gran Teocalli o Rancho Azteca)\nâ€¢	TransportaciÃ³n local durante la actividad en TeotihuacÃ¡n\nâ€¢	Despliegue de lona FELIZ ANIVERSARIO!\nâ€¢	Foto Impresa\nâ€¢	Coffee Break\n', NULL, NULL, NULL, NULL, NULL, 0.00, 7000.00, 0, 135.00, 1, 0, NULL, '2019-09-24 17:18:24', 4),
(1154, 9, NULL, 'JAVIER', 'NAVARRO NAVARRO', 'turismo@volarenglobo.com.mx', NULL, '5580284011', 11, 2, 0, 36, 3, '2019-10-29', NULL, NULL, NULL, NULL, NULL, '2 PAX COMPARTIDO CON TRANSPORTE TU EXPERIENCIA', NULL, NULL, NULL, NULL, 2, 200.00, 4700.00, 0, 0.00, 1, 0, NULL, '2019-09-24 17:23:01', 4),
(1155, 9, NULL, 'GRUPO', 'NOVIEMBRE', 'turismo@volarenglobo.com.mx', NULL, '5580284011', 11, 7, 0, 36, 3, '2019-11-10', NULL, NULL, NULL, NULL, NULL, '7 PAX COMPARTIDO  GALLEGOS TOURS', NULL, NULL, NULL, NULL, 2, 70.00, 14560.00, 0, 0.00, 1, 0, NULL, '2019-09-24 17:40:50', 4),
(1156, 16, '1139', 'NAYELLI', 'SIERRA', 'nayelli.sierra@hotmail.com', NULL, '5532356975', NULL, 4, 0, 38, 1, '2019-10-05', NULL, NULL, NULL, NULL, NULL, 'TE INCLUYE: \nâ€¢	VUELO LIBRE SOBRE EL VALLE DE TEOTIHUACÃN DE 45 A 60 MINUTOS \nâ€¢	BRINDIS CON VINO ESPUMOSO O JUGO DE FRUTA \nâ€¢	CERTIFICADO DE VUELO PERSONALIZADO \nâ€¢	SEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nâ€¢	COFFE BREAK: CAFE, TE, GALLETAS, PAN.\nâ€¢	DESPLIEGUE DE LONA FELIZ CUMPLE!\n\n', NULL, NULL, NULL, NULL, 2, 164.00, 9596.00, 0, 0.00, 1, 0, NULL, '2019-09-24 18:19:15', 4),
(1157, 14, NULL, 'JESUS', 'TAPIA', 'volarenglobo@yahoo.es', NULL, '2227663931', 11, 2, 0, 39, 4, '2019-10-13', NULL, NULL, NULL, NULL, NULL, 'VUELO PRIVADO PARA 2 PERSONAS , VUELO SOBRE VALLE DE TEOTIHUACAN DURANTE 45 MINS APROX , COFFE BREAK INCLUIDO (CAFÃ‰,TE,CAFÃ‰,PAN) SEGURO DE VIAJERO,CERTIFICADO PERSONALIZADO,TRANSPORTE DURANTE LA ACTIVIDAD,DESPLIEGUE DE LONA DE \"TE QUIERES CASAR CONMIGO\" DESAYUNO EN RESTAURANTE GRAN TEOCALLI Y FOTOGRAFIA IMPRESA A ELEGIR.', NULL, NULL, NULL, NULL, NULL, 0.00, 10100.00, 0, 0.00, 1, 0, NULL, '2019-09-24 18:28:48', 3),
(1158, 14, NULL, 'GONZALO', 'MORAN', 'volarenglobo@yahoo.es', NULL, '5522161041', 11, 2, 0, 36, 4, '2019-09-29', NULL, NULL, NULL, NULL, NULL, 'VUELO PRIVADO PARA 2 PERSONAS , VUELO SOBRE VALLE DE TEOTIHUACAN DURANTE 45 MINS APROX , COFFE BREAK INCLUIDO (CAFÃ‰,TE,CAFÃ‰,PAN) SEGURO DE VIAJERO,CERTIFICADO PERSONALIZADO,TRANSPORTE DURANTE LA ACTIVIDAD, BRINDIS CON VINO ESPUMOSO,DESAYUNO EN RESTAURANTE GRAN TEOCALLI Y FOTOGRAFIA IMPRESA A ELEGIR.', NULL, NULL, NULL, NULL, NULL, 0.00, 7500.00, 0, 133.00, 1, 0, NULL, '2019-09-25 12:59:20', 8),
(1159, 14, NULL, 'ANDREA', 'GUERRA', 'volarenglobo@yahoo.es', NULL, '50257041207', 35, 3, 0, 38, 1, '2019-09-26', NULL, NULL, NULL, NULL, NULL, 'VUELO COMPARTIDO PARA  3 PERSONAS, VUELO SOBRE EL VALLE DE TEOTIHUACAN, DURANTE 45 MINS APROX, INCLUYE COFFE BREAK (CAFÃ‰, TE, PAN GALLETAS) SEGURO DE VIAJERO, CERTIFICADO PERSONALIZADO, TRANSPORTE DURANTE LA ACTIVIDAD, BRINDIS CON VINO ESPUMOSO, DESPLIEGUE DE LONA,DESAYUNO Y TRANSPORTE REDONDO', 'DESAYUNOS', 297.00, NULL, NULL, NULL, 0.00, 8697.00, 0, 248.00, 1, 0, NULL, '2019-09-25 13:06:16', 7),
(1160, 16, NULL, 'CARLOS', 'GAMA', 'carl_yop@yahoo.com.mx', NULL, '4434100433', NULL, 10, 0, NULL, 2, '2019-10-20', NULL, NULL, NULL, NULL, NULL, 'TE INCLUYE: \nâ€¢	VUELO LIBRE SOBRE EL VALLE DE TEOTIHUACÃN DE 45 A 60 MINUTOS \nâ€¢	BRINDIS CON VINO ESPUMOSO O JUGO DE FRUTA \nâ€¢	CERTIFICADO DE VUELO PERSONALIZADO \nâ€¢	SEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nâ€¢	COFFE BREAK: CAFE, TE, GALLETAS, PAN.\nâ€¢	DESPLIEGUE DE LONA\nâ€¢	DESAYUNO BUFFETE EN RESTAURANTE GRAN TEOCALLI.\n', NULL, NULL, NULL, NULL, NULL, 0.00, 21400.00, 0, 0.00, 1, 0, NULL, '2019-09-25 14:00:52', 3),
(1161, 11, NULL, 'DARIO', 'BRAVO', 'volarenglobo@yahoo.es', NULL, '573136855276', 35, 2, 0, NULL, 1, '2019-09-29', NULL, NULL, NULL, NULL, NULL, 'INCLUYE\nVUELO LIBRE SOBRE TEOTIHUACAN DE 45 A 60 MINUTOS\nCOFFEE BREAK\nBRINDIS CON VINO ESPUMOSO PETILLANT DE FREIXENET\nCERTIFICADO DE VUELO\nSEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nTRANSPORTE IDA Y VUELTA DESDE HOTEL FIESTA INN AEROPUERTO', NULL, NULL, NULL, NULL, NULL, 0.00, 5600.00, 0, 125.00, 1, 0, NULL, '2019-09-25 14:36:15', 4),
(1162, 16, NULL, 'RICARDO', 'RODRIGUEZ', 'rodrigueztoledoricado10@gmail.com', NULL, '5568935326', NULL, 2, 0, 44, 1, '2019-09-29', NULL, NULL, NULL, NULL, NULL, 'TE INCLUYE: \nâ€¢	VUELO LIBRE SOBRE EL VALLE DE TEOTIHUACÃN DE 45 A 60 MINUTOS \nâ€¢	BRINDIS CON VINO ESPUMOSO O JUGO DE FRUTA \nâ€¢	CERTIFICADO DE VUELO PERSONALIZADO \nâ€¢	SEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nâ€¢	COFFE BREAK: CAFE, TE, GALLETAS, PAN.\nâ€¢	DESPLIEGUE DE LONA\nâ€¢	DESAYUNO BUFFETE EN RESTAURANTE GRAN TEOCALLI.\n', NULL, NULL, NULL, NULL, 2, 82.00, 4798.00, 0, 135.00, 1, 0, NULL, '2019-09-25 15:07:04', 4),
(1163, 16, NULL, 'MISSAEL', 'OLMEDO', 'ricardo.olmedo@live.com.mx', NULL, '5544200675', NULL, 2, 0, 36, 2, '2019-09-28', NULL, NULL, NULL, NULL, NULL, 'TE INCLUYE: \nâ€¢	VUELO LIBRE SOBRE EL VALLE DE TEOTIHUACÃN DE 45 A 60 MINUTOS \nâ€¢	BRINDIS CON VINO ESPUMOSO O JUGO DE FRUTA \nâ€¢	CERTIFICADO DE VUELO PERSONALIZADO \nâ€¢	SEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nâ€¢	COFFE BREAK: CAFE, TE, GALLETAS, PAN.\n', NULL, NULL, NULL, NULL, NULL, 0.00, 4000.00, 0, 135.00, 1, 0, NULL, '2019-09-25 15:21:20', 4),
(1164, 16, NULL, 'LAURA', 'REYNA', 'laurarsaenz@hotmail.com', NULL, '5554034929', NULL, 2, 0, 36, 1, '2019-09-28', NULL, NULL, NULL, NULL, NULL, 'TE INCLUYE: \nâ€¢	VUELO LIBRE SOBRE EL VALLE DE TEOTIHUACÃN DE 45 A 60 MINUTOS \nâ€¢	BRINDIS CON VINO ESPUMOSO O JUGO DE FRUTA \nâ€¢	CERTIFICADO DE VUELO PERSONALIZADO \nâ€¢	SEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nâ€¢	COFFE BREAK: CAFE, TE, GALLETAS, PAN.\nâ€¢	DESPLIEGUE DE LONA\nâ€¢	DESAYUNO BUFFETE EN RESTAURANTE GRAN TEOCALLI.\nâ€¢	TRANSPORTE REDONDO CDMX TEOTIHUACÃN\n', NULL, NULL, NULL, NULL, 2, 82.00, 5798.00, 0, 0.00, 1, 0, NULL, '2019-09-25 15:57:42', 8),
(1165, 16, NULL, 'DANIEL', 'MONTOYA', 'daniel@tiberica.com', NULL, '5517432060', NULL, 2, 2, 38, 1, '2019-09-28', NULL, NULL, NULL, NULL, NULL, 'TE INCLUYE: \nâ€¢	VUELO LIBRE SOBRE EL VALLE DE TEOTIHUACÃN DE 45 A 60 MINUTOS \nâ€¢	BRINDIS CON VINO ESPUMOSO O JUGO DE FRUTA \nâ€¢	CERTIFICADO DE VUELO PERSONALIZADO \nâ€¢	SEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nâ€¢	COFFE BREAK: CAFE, TE, GALLETAS, PAN.\nâ€¢	DESPLIEGUE DE LONA\n', NULL, NULL, NULL, NULL, NULL, 0.00, 8000.00, 0, 0.00, 1, 0, NULL, '2019-09-25 16:00:03', 4),
(1166, 16, NULL, 'JUAN PABLO', 'MARTINEZ', 'pablomartz@msn.com', NULL, '4431645078', NULL, 6, 5, 43, 1, '2019-09-28', NULL, NULL, NULL, NULL, NULL, 'TE INCLUYE: \nâ€¢	VUELO LIBRE SOBRE EL VALLE DE TEOTIHUACÃN DE 45 A 60 MINUTOS \nâ€¢	BRINDIS CON VINO ESPUMOSO O JUGO DE FRUTA \nâ€¢	CERTIFICADO DE VUELO PERSONALIZADO \nâ€¢	SEGURO DE VIAJERO DURANTE LA ACTIVIDAD\nâ€¢	COFFE BREAK: CAFE, TE, GALLETAS, PAN.\nâ€¢	DESPLIEGUE DE LONA\n', NULL, NULL, NULL, NULL, 2, 1200.00, 21100.00, 0, 0.00, 1, 0, NULL, '2019-09-25 16:25:56', 4),
(1167, 16, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0.00, 0.00, 0, 0.00, 1, 0, NULL, '2019-09-25 18:12:49', 2),
(1168, 14, NULL, 'ANA MARAI', 'ESTUPIÃ‘AN', 'volarenglobo@yahoo.es', NULL, '573144768664', 35, 4, 0, 36, 1, '2019-09-26', NULL, NULL, NULL, NULL, NULL, 'VUELO COMPARTIDO PARA 4 PERSONAS, VUELO SOBRE EL VALLE DE TEOTIHUACAN, DURANTE 45 MINS APROX, INCLUYE COFFE BREAK (CAFÃ‰, TE, PAN GALLETAS) SEGURO DE VIAJERO, CERTIFICADO PERSONALIZADO, TRANSPORTE DURANTE LA ACTIVIDAD, BRINDIS CON VINO ESPUMOSO', NULL, NULL, NULL, NULL, 2, 1200.00, 8000.00, 0, 0.00, 1, 0, NULL, '2019-09-25 18:21:02', 6),
(1169, 14, '1168', 'ANA MARIA', 'ESTUPIÃ‘AN', 'volarenglobo@yahoo.es', NULL, '573144768664', 35, 4, 0, 36, 1, '2019-09-26', NULL, NULL, NULL, NULL, NULL, 'VUELO COMPARTIDO PARA 4 PERSONAS, VUELO SOBRE EL VALLE DE TEOTIHUACAN, DURANTE 45 MINS APROX, INCLUYE COFFE BREAK (CAFÃ‰, TE, PAN GALLETAS) SEGURO DE VIAJERO, CERTIFICADO PERSONALIZADO, TRANSPORTE DURANTE LA ACTIVIDAD, BRINDIS CON VINO ESPUMOSO', NULL, NULL, NULL, NULL, 2, 1200.00, 8000.00, 0, 293.00, 1, 0, NULL, '2019-09-25 18:22:50', 4),
(1170, 16, NULL, 'DIANEY', 'SAHAGUN', 'Dianey.sahagun@outlook.es', NULL, '3333991673', NULL, 2, 0, NULL, 5, '2019-10-01', NULL, NULL, NULL, NULL, NULL, 'â€¢	Vuelo en globo exclusivo\nâ€¢	Brindis en vuelo con vino espumoso Freixenet\nâ€¢	Certificado de vuelo\nâ€¢	Seguro viajero\nâ€¢	Desayuno tipo Buffet (Restaurante Gran Teocalli o Rancho Azteca)\nâ€¢	TransportaciÃ³n local durante la actividad en TeotihuacÃ¡n\nâ€¢	Despliegue de lona \nâ€¢	Foto Impresa\nâ€¢	Coffee Break\n', NULL, NULL, NULL, NULL, NULL, 0.00, 6500.00, 0, 0.00, 1, 0, NULL, '2019-09-25 18:57:56', 8);
...
--
ALTER TABLE `bitpagos_volar`
  MODIFY `id_bp` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Llave Primaria', AUTO_INCREMENT=105;
...
--
ALTER TABLE `servicios_vuelo_temp`
  MODIFY `id_sv` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Llave Primaria', AUTO_INCREMENT=2638;
...
--
ALTER TABLE `temp_volar`
  MODIFY `id_temp` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Llave Primaria', AUTO_INCREMENT=1171;
...
-- Eventos
--
CREATE DEFINER=`afraqujt`@`localhost` EVENT `cancelaReservas` ON SCHEDULE EVERY 1 DAY STARTS '2019-08-28 01:47:37' ON COMPLETION PRESERVE ENABLE COMMENT 'Cancelar reservas que tengan mas de 30 dias' DO Update temp_volar set status = 0 where  fechavuelo_temp < DATE_SUB(NOW(), INTERVAL 30 DAY) and status not in(1,7)$$

CREATE DEFINER=`afraqujt`@`localhost` EVENT `cancelarReservasSinCot` ON SCHEDULE EVERY 1 MINUTE STARTS '2019-09-03 12:29:00' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE temp_volar set status=0 where status=2$$
